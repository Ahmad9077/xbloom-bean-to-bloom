...
export interface RecommendationJobRow {
  id: string;
  owner_id: string;
  username_display: string;
  bean_name: string;
...
export async function completeBridgeJob(
  ...
  await db
    ...
    .bind(status, now, now, safeError, shareLink, id)
    .run();
}

// ---------------------------------------------------------------------------
// Codex recommendation jobs
// ---------------------------------------------------------------------------
  ...
  await db
    .prepare(
      `INSERT INTO recommendation_jobs
       (id, owner_id, username_display, bean_name, bean_json, brew_mode, status, attempts, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, 'pending', 0, ?, ?)`,
    )
    .bind(
      data.id,
...
export async function getRecommendationJobById(
  db: D1Database,
  id: string,
): Promise<RecommendationJobRow | null> {
  return db
    .prepare("SELECT * FROM recommendation_jobs WHERE id = ?")
    .bind(id)
    .first<RecommendationJobRow>();
}

const RECOMMENDATION_STALE_MS = 5 * 60 * 1000;
const RECOMMENDATION_MAX_ATTEMPTS = 2;

export async function claimNextRecommendationJob(
  db: D1Database,
): Promise<RecommendationJobRow | null> {
  const now = Date.now();
  await db
    .prepare(
      `UPDATE recommendation_jobs SET status = 'pending', claimed_at = NULL, updated_at = ?
       WHERE status = 'claimed' AND claimed_at < ?`,
    )
    .bind(now, now - RECOMMENDATION_STALE_MS)
    .run();
  await db
    .prepare(
      `UPDATE recommendation_jobs SET status = 'failed', completed_at = ?, updated_at = ?, safe_error = ?
       WHERE status = 'pending' AND attempts >= ?`,
    )
    .bind(
      now,
      now,
      "The AI recommendation could not be completed after several attempts.",
      RECOMMENDATION_MAX_ATTEMPTS,
    )
    .run();
  return (
    (await db
      .prepare(
        `UPDATE recommendation_jobs
         ...
         WHERE id = (
           ...
           WHERE status = 'pending' AND attempts < ? ORDER BY created_at ASC LIMIT 1
         ) RETURNING *`,
      )
      .bind(now, now, RECOMMENDATION_MAX_ATTEMPTS)
      .first<RecommendationJobRow>()) ?? null
  );
}

export async function failRecommendationJob(
  db: D1Database,
  id: string,
  safeError: string,
): Promise<void> {
  ...
  await db
    .prepare(
      `UPDATE recommendation_jobs
       SET status = 'failed', safe_error = ?, completed_at = ?, updated_at = ?
       WHERE id = ? AND status = 'claimed'`,
    )
    .bind(safeError, now, now, id)
    .run();
}

export async function completeRecommendationJob(
  db: D1Database,
  job: RecommendationJobRow,
  recipe: {
    id: string;
    fullName: string;
    beanName: string;
  ...
  await db.batch([
    db
      ...
      .bind(recipe.id, job.owner_id, recipe.fullName, recipe.beanName, recipe.recipeJson, now, now),
    db
      .prepare(
        `UPDATE recommendation_jobs
         SET status = 'completed', recipe_id = ?, completed_at = ?, updated_at = ?, safe_error = NULL
         WHERE id = ? AND status = 'claimed'`,
      )
      .bind(recipe.id, now, now, job.id),
  ]);
}

export async function pruneOldRecommendationJobs(db: D1Database): Promise<void> {
  const cutoff = Date.now() - 7 * 24 * 60 * 60 * 1000;
  await db
    .prepare(
      "DELETE FROM recommendation_jobs WHERE status IN ('completed', 'failed') AND completed_at < ?",
    )
    .bind(cutoff)
    .run();
}
